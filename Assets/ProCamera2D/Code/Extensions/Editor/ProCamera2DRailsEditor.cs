...
namespace Com.LuisPedroFonseca.ProCamera2D
{
    ...
    public class ProCamera2DRailsEditor : Editor
    {
        ...
        void OnSceneGUI()
        {
            ...
            if (Handles.Button(rightHandlePos, Quaternion.identity, handleSize * .5f, handleSize, Handles.RectangleHandleCap))
            {
            ...
            if (Handles.Button(leftHandlesPos, Quaternion.identity, handleSize * .5f, handleSize, Handles.RectangleHandleCap))
            {
                Undo.RecordObject(proCamera2DRails, "Add");
            ...
            for (int i = 0; i < nodesCount; i++)
            {
                ...
                var fmh_304_34_638950315195912235 = Quaternion.identity; var newPos = Handles.FreeMoveHandle(
                                 oldPos,
                                 handleSize,
                                 pointSnap,
                ...
                if (i > 0)
                {
                    ...
                    if (Handles.Button(midPoint, Quaternion.identity, handleSize * .5f, handleSize, Handles.RectangleHandleCap))
                    {
                        Undo.RecordObject(proCamera2DRails, "Add");
                ...
                if (deleteMode && nodesCount > 2)
                {
                    ...
                    if (Handles.Button(deleteButtonPos, Quaternion.identity, handleSize, handleSize, Handles.DotHandleCap))
                    {
